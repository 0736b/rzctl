...
void rzctl::mouse_click(int click_mask)
{
	RZCONTROL_IOCTL_STRUCT mm = {
		0, RZCONTROL_MOUSE,
		0, click_mask,
...
void rzctl::keyboard_input(short scan_code, KEYBOARD_INPUT_TYPE up_down)
{
	RZCONTROL_IOCTL_STRUCT mm = {
		0, RZCONTROL_KEYBOARD,
		// scan code is stored in left bits
		((int)scan_code) << 16, up_down,
		0, 0,
		0, 0,
	};
	_impl_mouse_ioctl(&mm);
...
void rzctl::_impl_mouse_ioctl(RZCONTROL_IOCTL_STRUCT* pMi)
{
	if (pMi)
	{
		DWORD junk;

		BOOL bResult = DeviceIoControl(hDevice, IOCTL_MOUSE, pMi, sizeof(RZCONTROL_IOCTL_STRUCT), NULL, 0, &junk, NULL);
